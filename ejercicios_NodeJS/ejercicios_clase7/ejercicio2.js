...
const server = net.createServer((socket) => {
    ...
    socket.on('data', (data) => {
        console.log('Los datos recibidos del cliente son: ', data.toString());
        socket.setTimeout(10000);
    });

    socket.on('timeout', () => {
        console.log('Tiempo de espera para recepción de datos alcanzado');
        socket.end();
    });
...
server.listen(3000, () => {
    console.log('Servidor TCP escuchando desde el puerto 3000');
});
